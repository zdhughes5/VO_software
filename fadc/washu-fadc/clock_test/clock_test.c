/*
      clock_test.c                                                            
      9/28/05                                                         
      Test routines for the CLK/TRIG board testing  
      ...
      CLOCK/TRIG BOARD mapped to 0xf0ff0000 (A32)
*/
...
int main(void)
{
	...
	printf("     4. software telescope trig    l. crate cLear\n");
	printf("     5. software crate trig        s. clear scalers\n");
	...
	case '4':
	    ...
	    *lptr = 0;
	    printf("Sent software telescope trig\n");
	    break;
	...
	case '2': /* test scalers */
		...
		*lptr = 0;
		...
		printf("%f sec ", value/32.0e9);
		if ((value/32.0e9) > 0.006) {
...
print_clk_headers(unsigned long *clkbd_lptr) {
    ...
    rdvalue=*lptr;
    printf("GPS sec mark  : 0x%08lx (%lu)\n", rdvalue,rdvalue);
    lptr = clkbd_lptr+0x2004;	    
    rdvalue=*lptr;
    printf("elapsed time  : 0x%08lx (%lu)\n", rdvalue,rdvalue);
    lptr = clkbd_lptr+0x2005;	    
    rdvalue=*lptr;
    printf("live time     : 0x%08lx (%lu)\n", rdvalue,rdvalue);
    lptr = clkbd_lptr+0x2007;	    
    rdvalue=*lptr;
