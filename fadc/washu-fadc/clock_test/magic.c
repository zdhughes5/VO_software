/*
      ...
      Magic board mapped to 0xf0fe0000 (A32)
...
{
    register int i;
    unsigned long *magic_lptr;   /* pointer for D32 CLKBD transfers    */
    ...
    lptr = magic_lptr + 0x2000;   /* write event clear */
    *lptr = 0;
    lptr = magic_lptr + 0x2002;   /* write crate clear */
    *lptr = 0;
	...
	case 'c':  /* evt clr */
	    lptr = magic_lptr + 0x2000; /* SOFT_EVT_CLR; */
	    *lptr = 0;
	...
	case '1': /* test writable registers */
	    ...
	    wrlptr = magic_lptr + 0x2001;
	    rdlptr = magic_lptr + 0x2000;
	    test_register( wrlptr, rdlptr, 8, 0xff00, 8 );

	    printf("Checking trigger code register (16 bits)...");
	    wrlptr = magic_lptr + 0x2003;
	    rdlptr = magic_lptr + 0x2002;
...
void
print_clk_headers(unsigned long *magic_lptr) {
    ...
    lptr = magic_lptr+0x2001; 	    
    rdvalue=*lptr;
    printf("Event number  : 0x%08lx (%ld)\n", rdvalue,rdvalue);

    lptr = magic_lptr+0x2002;	    
    rdvalue=*lptr;
    /* the first 16 bits are the programmable register, and bits 16-31 are 
       the rest of the event number (bits 32-47)
       (we'll call it the Event Type and Trig Mode */
    printf("Event type & Trig mode: 0x%04lx\n", rdvalue>>16);
    printf("programmable register: 0x%04lx\n", rdvalue&0x0000FFFF);
